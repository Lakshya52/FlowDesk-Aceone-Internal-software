...
const getConversations = async (req, res) => {
    try {
        ...
        const results = await Promise.all(conversations.map(async (conv) => {
            // Find last message
            const lastMessage = await Message_1.default.findOne({ conversation: conv._id })
                .sort({ createdAt: -1 })
            ...
            let isOnline = false;
            if (conv.type === Conversation_1.ConversationType.DIRECT) {
            ...
            if (lastMessage && lastMessage.content && !lastMessage.isDeleted) {
                lastMessage.content = (0, encryption_1.decrypt)(lastMessage.content);
            }
            return {
                _id: conv._id,
                type: conv.type,
        ...
        const filteredResults = results.filter(c => {
            if (c.type === Conversation_1.ConversationType.DIRECT) {
...
const getMessages = async (req, res) => {
    try {
        ...
        messages.forEach((msg) => decryptMessage(msg));
...
const createConversation = async (req, res) => {
    try {
        ...
        if (type === Conversation_1.ConversationType.DIRECT) {
            ...
            if (existing) {
                ...
                const lastMessage = await Message_1.default.findOne({ conversation: existing._id })
                    .sort({ createdAt: -1 })
                    .populate('sender', 'name email avatar')
                    .populate('attachments');
                ...
                const isOnline = index_1.activeUsers.has(otherParticipant._id.toString());
                res.status(200).json({
                    conversation: {
                        _id: existing._id,
                        type: existing.type,
        ...
        const conversation = await Conversation_1.default.create({
            type,
            name: type === Conversation_1.ConversationType.GROUP ? (name || 'New Group') : undefined,
            avatar: type === Conversation_1.ConversationType.GROUP ? (avatar || '') : undefined,
            participants: participantIds,
            createdBy: type === Conversation_1.ConversationType.GROUP ? userId : undefined,
            admins: type === Conversation_1.ConversationType.GROUP ? [userId] : undefined
        });
        const populated = await Conversation_1.default.findById(conversation._id)
            .populate('participants', 'name email avatar role employeeId isActive');
        if (!populated) {
        ...
        let isOnline = false;
        if (populated.type === Conversation_1.ConversationType.DIRECT) {
            const otherParticipant = populated.participants.find(p => p._id.toString() !== userId.toString()) || populated.participants[0];
            formattedName = otherParticipant.name;
            formattedAvatar = otherParticipant.avatar;
            isOnline = index_1.activeUsers.has(otherParticipant._id.toString());
        }
        ...
        participantIds.forEach(pId => {
            index_1.io.to(`user_${pId}`).emit('new_conversation', formattedResult);
        });
...
const sendMessage = async (req, res) => {
    try {
        ...
        if (mentions && Array.isArray(mentions)) {
            const mentionPromises = mentions.map((mId) => {
                if (mId === senderId.toString())
                    return Promise.resolve(); // Don't notify self
                return (0, notificationService_1.createNotification)({
                    user: mId,
                    type: Notification_1.NotificationType.DIRECT_MESSAGE,
                    title: 'New Mention',
...
const toggleReaction = async (req, res) => {
    try {
        ...
        await message.save();
        const updated = await Message_1.default.findById(messageId)
        ...
        index_1.io.to(`conversation_${message.conversation}`).emit('message_reaction_updated', socketPayload);
        // Also emit to user rooms to update lists if needed
        conversation.participants.forEach(pId => {
            index_1.io.to(`user_${pId.toString()}`).emit('message_reaction_updated', socketPayload);
        });
...
const deleteConversation = async (req, res) => {
    try {
        ...
        const messages = await Message_1.default.find({ conversation: conversationId }).populate('attachments');
        for (const msg of messages) {
            if (msg.attachments && msg.attachments.length > 0) {
                for (const att of msg.attachments) {
                    try {
                        await (0, gridfs_1.deleteFromGridFS)(att.fileName);
                        await Attachment_1.default.findByIdAndDelete(att._id);
                    }
                    catch (err) {
        ...
        await Conversation_1.default.findByIdAndDelete(conversationId);
        // Delete all messages belonging to this conversation
        await Message_1.default.deleteMany({ conversation: conversationId });
        // Notify all participants so it disappears from their sidebar in real-time
        conversation.participants.forEach(pId => {
            index_1.io.to(`user_${pId.toString()}`).emit('conversation_deleted', conversationId);
        });
...
const deleteMessage = async (req, res) => {
    try {
        const { messageId } = req.params;
        const userId = req.user._id;
        const message = await Message_1.default.findById(messageId).populate('attachments');
        if (!message) {
            res.status(404).json({ message: 'Message not found' });
            return;
        }
        ...
        if (conversation) {
            ...
            conversation.participants.forEach(pId => {
                index_1.io.to(`user_${pId.toString()}`).emit('message_deleted', socketPayload);
            });
        }
        res.json({ message });
    }
...
const forwardMessage = async (req, res) => {
    try {
        ...
        const originalMessage = await Message_1.default.findById(messageId).populate('attachments');
        if (!originalMessage) {
        ...
        const targetConversation = await Conversation_1.default.findById(targetConversationId);
        ...
        const populated = await Message_1.default.findById(forwarded._id)
            .populate('sender', 'name email avatar role')
            .populate('attachments')
            .populate({
            path: 'parentMessage',
...
const editMessage = async (req, res) => {
    try {
        ...
        if (conversation) {
            conversation.participants.forEach(pId => {
                index_1.io.to(`user_${pId.toString()}`).emit('message_edited', populated);
            });
        }
