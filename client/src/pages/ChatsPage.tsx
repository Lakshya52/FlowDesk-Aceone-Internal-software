...
function DeleteConfirm({ label, onConfirm, onCancel }: { label: string; onConfirm: () => void; onCancel: () => void }) {
    return (
        <div style={{
            ...
            <button onClick={onConfirm} style={{ border: 'none', background: 'var(--color-danger)', color: 'white', borderRadius: 6, padding: '2px 8px', cursor: 'pointer', fontWeight: 700, fontSize: '10px' }}>Yes</button>
            <button onClick={onCancel} style={{ border: '1px solid var(--color-border)', background: 'none', color: 'var(--color-text-secondary)', borderRadius: 6, padding: '2px 8px', cursor: 'pointer', fontSize: '10px' }}>No</button>
        </div>
    );
}
...
export default function ChatsPage() {
    ...
    useEffect(() => {
        ...
        socket.on('user_chat_typing', ({ conversationId, userName, userId }: any) => {
            if (conversationId === currentConvRef.current && userId !== user?._id) {
                setTypingUsers(prev => prev.some(u => u.id === userId) ? prev : [...prev, { id: userId, name: userName }]);
            }
        });
        ...
        socket.on('message_edited', (editedMessage: MessageSnippet) => {
            if (editedMessage.conversation === currentConvRef.current) {
                ...
                // Also update cache
                messagesCacheRef.current[editedMessage.conversation] = (messagesCacheRef.current[editedMessage.conversation] || []).map(
                    m => m._id === editedMessage._id ? editedMessage : m
                );
            }
        ...
        socket.on('messages_read', ({ conversationId, readerId, readAt }: {
            ...
            if (conversationId === currentConvRef.current) {
                setMessages(prev => prev.map(m => {
                    ...
                    return {
                        ...m,
                        readBy: [...(m.readBy || []), { user: readerId, readAt: new Date(readAt) }]
                    };
                }));
            }
        });
    ...
    useEffect(() => {
        ...
        const fetchMessages = async () => {
            try {
                ...
                const conv = useChatStore.getState().conversations.find(c => c._id === activeConversationId);
    ...
    const selectMention = (selectedUser: UserSnippet) => {
        ...
        const after = messageInput.substring(mentionCursorPos + mentionFilter.length + 1);
    ...
    const handleSendMessage = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            if (matchingMentions.length > 0) formData.append('mentions', JSON.stringify(matchingMentions));
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (rejected.length > 0) toast.error('Files not uploaded:\n\n' + rejected.join('\n'));
                        ...
                        const { data } = await api.post(
                            ...
                            formData,
                            {
                                ...
                                onUploadProgress: (progressEvent) => {
                                    ...
                                    setUploadingQueue(prev => prev.map(item => item.id === queueId ? { ...item, progress: pct } : item));
                                }
                            }
                        );
                        setUploadingQueue(prev => prev.map(item => item.id === queueId ? { ...item, progress: 100, status: 'completed' } : item));
                        setMessages(prev => prev.some(m => m._id === data.message._id) ? prev : [...prev, data.message]);
                    } catch (uploadError) {
                        setUploadingQueue(prev => prev.map(item => item.id === queueId ? { ...item, status: 'failed' } : item));
                        throw uploadError;
                    }
                })
            );
    ...
    const handleEditMessage = async (messageId: string) => {
        ...
        try {
            const { data } = await api.put(`/conversations/messages/${messageId}`, { content: editMessageInput.trim() });
    ...
    const handleStartDirectChat = async (targetUserId: string) => {
        try {
            const { data } = await api.post('/conversations', { type: 'direct', participants: [targetUserId] });
    ...
    const matchesNameStart = (name: string, query: string) => {
        ...
        return name.toLowerCase().split(/\s+/).some(p => p.startsWith(lq));
    };

    const filteredConversations = conversations.filter(c => matchesNameStart(c.name, searchQuery));
    ...
    const matchingExternalUsers = searchQuery.trim() ? users.filter(u => {
        ...
        return !inConversations && u._id !== user?._id && matchesNameStart(u.name, searchQuery);
    }) : [];
    ...
    return (
        <>
            ...
            <div style={{
                        ...
                        {isLoading ? (
                            ...
                            <>
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    style={{
                                                        ...
                                                        gap: 12,
                                                        cursor: 'pointer',
                                                        background: isActive ? 'var(--color-primary-light)' : (isHovered ? 'var(--color-surface-hover)' : 'transparent'),
                                                    ...
                                                    <div style={{ flex: 1, minWidth: 0 }}>
                                                        ...
                                                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                                            <p style={{
                                                                ...
                                                                margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '85%',
                                                            }}>
                                                                {latestMsg ? (
                                                                    <>{latestMsg.sender._id === user?._id ? 'You: ' : `${latestMsg.sender.name}: `}{latestMsg.content || 'Sent an attachment'}</>
                                                                ) : 'No messages yet'}
                                                            </p>
                                                            {c.unreadCount > 0 && (
                                                                <span style={{
                                ...
                                {matchingExternalUsers.length > 0 && (
                                    <div style={{ paddingTop: 12, borderTop: '1px solid var(--color-border)', marginTop: 8, display: 'flex', flexDirection: 'column', gap: 4 }}>
                                        <h5 style={{ padding: '0 8px', fontSize: '10px', fontWeight: 700, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 6px 0' }}>
                                            Start new chat
                                        </h5>
                                        {matchingExternalUsers.map(u => (
                                            <div
                                                ...
                                                style={{
                                                    padding: 10, borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12, cursor: 'pointer',
                                                    background: hoveredContactId === u._id ? 'var(--color-surface-hover)' : 'transparent',
                                                    transition: 'background 0.2s',
                                                }}
                                            >
                                                <Avatar src={u.avatar} name={u.name} size={32} />
                                                <div>
                                                    <h4 style={{ fontSize: '0.8125rem', fontWeight: 600, margin: 0, color: 'var(--color-text)' }}>{u.name}</h4>
                                                    <p style={{ fontSize: '10px', color: 'var(--color-text-tertiary)', margin: 0, textTransform: 'capitalize' }}>{u.role}</p>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                {filteredConversations.length === 0 && matchingExternalUsers.length === 0 && (
                                    <div style={{ padding: '32px 16px', textAlign: 'center', color: 'var(--color-text-tertiary)', fontSize: '0.75rem' }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--color-bg)', minWidth: 0 }}>
                    {activeConv ? (
                        <>
                            {/* Conversation Header */}
                            <div style={{
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    ...
                                    <div>
                                        <h3 style={{ fontSize: '0.875rem', fontWeight: 700, margin: 0, color: 'var(--color-text)' }}>
                                            {activeConv.name}
                                        </h3>
                                        <p style={{ fontSize: '10px', color: 'var(--color-text-tertiary)', margin: 0, fontWeight: 500 }}>
                                ...
                                <div style={{ position: 'relative' }}>
                                    {pendingDeleteConv ? (
                                        ...
                                        <button
                                            ...
                                            style={{
                                                border: 'none', background: 'none', cursor: 'pointer',
                                                color: 'var(--color-text-tertiary)', padding: '8px', borderRadius: '50%',
                            ...
                            <div style={{ flex: 1, overflowY: 'auto', padding: '24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
                                {loadingMessages ? (
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, justifyContent: 'center', alignItems: 'center', minHeight: '200px', height: '100%' }}>
                                        <div style={{ width: 48, height: 48, borderRadius: '50%', border: '3px solid var(--color-surface-hover)', borderTopColor: 'var(--color-primary)', animation: 'chatsPageSpinner 1s linear infinite' }} />
                                        <span style={{ fontSize: '0.875rem', color: 'var(--color-text-secondary)', fontWeight: 500 }}>Loading your messages...</span>
                                    </div>
                                ) : messages.length === 0 ? (
                                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, color: 'var(--color-text-tertiary)', fontSize: '0.875rem', textAlign: 'center', padding: '40px 20px' }}>
                                            ...
                                            <div key={msg._id} style={{ display: 'flex', flexDirection: 'column' }}>
                                                ...
                                                <div
                                                    ...
                                                    style={{
                                                        ...
                                                        alignItems: 'flex-end',
                                                        gap: 8, width: '100%', position: 'relative',
                                                    }}
                                                >
                                                                ...
                                                                <>
                                                                    ...
                                                                    <div style={{ position: 'relative' }}>
                                                                        <button
                                                                            ...
                                                                            style={{ border: 'none', background: 'none', cursor: 'pointer', padding: '4px', color: 'var(--color-text-tertiary)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}
                                                                            onMouseEnter={(e) => e.currentTarget.style.background = 'var(--color-surface-hover)'}
                                                                            onMouseLeave={(e) => e.currentTarget.style.background = 'none'}
                                                                            title="React"
                                                                        >
                                                                        ...
                                                                        {activeReactionPickerId === msg._id && (
                                                                            <div style={{ position: 'absolute', bottom: '100%', right: 0, display: 'flex', alignItems: 'center', gap: 6, padding: '6px 8px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 12, boxShadow: 'var(--shadow-lg)', zIndex: 60, whiteSpace: 'nowrap', marginBottom: 6 }}>
                                                                                {emojis.map(e => (
                                                                                    <button key={e} onClick={() => { handleReactToMessage(msg._id, e); setActiveReactionPickerId(null); }} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: '2px 4px', fontSize: '1rem', transition: 'transform 0.1s' }}
                                                                    ...
                                                                    {/* Forward */}
                                                                    <button
                                                                        onClick={() => { setForwardingMessage(msg); setForwardSuccessConvIds([]); }}
                                                                        style={{ border: 'none', background: 'none', cursor: 'pointer', padding: '4px', color: 'var(--color-text-tertiary)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}
                                                                        onMouseEnter={(e) => e.currentTarget.style.background = 'var(--color-surface-hover)'}
                                                                        onMouseLeave={(e) => e.currentTarget.style.background = 'none'}
                                                                        title="Forward"
                                                                    ...
                                                                    {!msg.isDeleted && (
                                                                        <button
                                                                            onClick={() => { setEditingMessageId(msg._id); setEditMessageInput(msg.content); }}
                                                                            style={{ border: 'none', background: 'none', cursor: 'pointer', padding: '4px', color: 'var(--color-text-tertiary)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}
                                                                            onMouseEnter={(e) => e.currentTarget.style.background = 'var(--color-surface-hover)'}
                                                                            onMouseLeave={(e) => e.currentTarget.style.background = 'none'}
                                                                            title="Edit"
                                                                    ...
                                                                    {!msg.isDeleted && (
                                                                        <button
                                                                            onClick={() => setPendingDeleteMsgId(msg._id)}
                                                                            style={{ border: 'none', background: 'none', cursor: 'pointer', padding: '4px', color: 'var(--color-danger)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.2s' }}
                                                    ...
                                                    <div style={{ position: 'relative', maxWidth: '70%' }}>
                                                        ...
                                                        {!isMe && isGroup && (
                                                            <p style={{ fontSize: '10px', fontWeight: 700, color: 'var(--color-primary)', margin: '0 0 3px 4px' }}>
                                                                {msg.sender.name}
                                                            </p>
                                                        )}
                                                        ...
                                                        {/* Reply context */}
                                                        {msg.parentMessage && (
                                                            <div style={{ marginBottom: 4, padding: 8, borderRadius: 10, background: 'rgba(0,0,0,0.05)', borderLeft: '4px solid var(--color-border)', fontSize: '11px', color: 'var(--color-text-secondary)' }}>
                                                        ...
                                                        <div style={{
                                                            ...
                                                            {msg.isDeleted ? (
                                                                <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontStyle: 'italic', opacity: 0.7, fontSize: '0.8125rem', color: isMe ? 'rgba(255,255,255,0.8)' : 'var(--color-text-tertiary)' }}>
                                                                    <Ban size={14} />
                                                                ...
                                                                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                                                    <textarea
                                                                        className="input"
                                                                        style={{ width: '100%', minWidth: 200, height: 60, background: isMe ? 'rgba(255,255,255,0.15)' : 'var(--color-bg)', color: isMe ? 'white' : 'var(--color-text)', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 8, fontSize: '0.875rem', padding: 8, resize: 'none' }}
                                                                        value={editMessageInput}
                                                                        onChange={(e) => setEditMessageInput(e.target.value)}
                                                                        autoFocus
                                                                    />
                                                                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6 }}>
                                                                        <button onClick={() => { setEditingMessageId(null); setEditMessageInput(''); }} className="btn btn-ghost" style={{ height: 24, padding: '0 8px', fontSize: '10px', color: isMe ? 'rgba(255,255,255,0.8)' : 'var(--color-text-secondary)' }}>Cancel</button>
                                                                        <button onClick={() => handleEditMessage(msg._id)} className="btn btn-primary" style={{ height: 24, padding: '0 12px', fontSize: '10px', borderRadius: 6, background: isMe ? 'white' : 'var(--color-primary)', color: isMe ? 'var(--color-primary)' : 'white', border: 'none', fontWeight: 700 }}>Save</button>
                                                                    </div>
                                                                </div>
                                                            ) : (
                                                                <>
                                                                    {msg.attachments && msg.attachments.length > 0 && (
                                                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 6 }}>
                                                                            {msg.attachments.map(att => {
                                                                                ...
                                                                                if (isImage) {
                                                                                    return (
                                                                                        <div key={att._id} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', maxWidth: 220 }}>
                                                                                            ...
                                                                                            <a href={fileUrl} download={att.originalName} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', textDecoration: 'none' }}>
                                                                                                <Download size={18} />
                                                                                            </a>
                                                                                        </div>
                                                                                    );
                                                                                ...
                                                                                return (
                                                                                    <div key={att._id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 8, borderRadius: 8, background: isMe ? 'rgba(255,255,255,0.15)' : 'var(--color-bg)', border: '1px solid rgba(0,0,0,0.05)', color: 'inherit' }}>
                                                                                        ...
                                                                                        <a href={fileUrl} download={att.originalName} style={{ color: 'inherit', display: 'flex' }}><Download size={14} /></a>
                                                                                    </div>
                                                                                );
                                                                            })}
                                                                        </div>
                                                                    ...
                                                                    {msg.content && (
                                                                        <p style={{ margin: 0, lineHeight: 1.5, whiteSpace: 'pre-wrap' }}>
                                                                            ...
                                                                            {msg.isEdited && (
                                                                                <span style={{ fontSize: '10px', opacity: 0.7, marginLeft: 6, fontStyle: 'italic', color: 'inherit', display: 'inline-block' }}>(edited)</span>
                                                                            )}
                                                                        </p>
                                                                    )}
                                                                </>
                                                            ...
                                                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 4, marginTop: 4 }}>
                                                                <span style={{ fontSize: '9px', opacity: 0.7, color: isMe ? 'rgba(255,255,255,0.8)' : 'var(--color-text-tertiary)' }}>
                                                            ...
                                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginTop: 4, justifyContent: isMe ? 'flex-end' : 'flex-start' }}>
                                                                {Object.entries(
                                                                    ...
                                                                    <button
                                                                        ...
                                                                        style={{ display: 'flex', alignItems: 'center', gap: 2, padding: '2px 6px', borderRadius: 12, background: 'var(--color-surface)', border: '1px solid var(--color-border)', fontSize: '10px', cursor: 'pointer', boxShadow: 'var(--shadow-sm)' }}
                                                    ...
                                                    {!isMe && (
                                                        <div style={{
                                                            ...
                                                            <div style={{ position: 'relative' }}>
                                                                ...
                                                                {activeReactionPickerId === msg._id && (
                                                                    <div style={{ position: 'absolute', bottom: '100%', left: 0, display: 'flex', alignItems: 'center', gap: 6, padding: '6px 8px', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 12, boxShadow: 'var(--shadow-lg)', zIndex: 60, whiteSpace: 'nowrap', marginBottom: 6 }}>
                                                                        {emojis.map(e => (
                                                                            <button key={e} onClick={() => { handleReactToMessage(msg._id, e); setActiveReactionPickerId(null); }} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: '2px 4px', fontSize: '1rem', transition: 'transform 0.1s' }}
                                                                                onMouseEnter={(e) => e.currentTarget.style.transform = 'scale(1.25)'}
                                                                                onMouseLeave={(e) => e.currentTarget.style.transform = 'scale(1)'}
                            ...
                            {/* COMPOSER PANEL */}
                            <div style={{ padding: 16, borderTop: '1px solid var(--color-border)', background: 'var(--color-surface)', display: 'flex', flexDirection: 'column', gap: 8, position: 'relative' }}>

                                {/* Mentions autocomplete */}
                                {showMentions && (
                                    <div style={{ position: 'absolute', bottom: '100%', left: 16, marginBottom: 8, width: 240, background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 12, boxShadow: 'var(--shadow-lg)', maxHeight: 180, overflowY: 'auto', padding: 6, zIndex: 50 }}>
                                        <div style={{ padding: '4px 8px', fontSize: '9px', fontWeight: 700, color: 'var(--color-text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid var(--color-border)', marginBottom: 4 }}>
                                            Mention Team Member
                                        </div>
                                        {activeConv.participants
                                            .filter((p: any) => p._id !== user?._id && p.name.toLowerCase().includes(mentionFilter))
                                            .map((p: any) => (
                                                <div key={p._id} onClick={() => selectMention(p)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: 6, cursor: 'pointer', borderRadius: 8 }}
                                                    onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--color-surface-hover)'; }}
                                ...
                                {isUploadingFile && uploadingQueue.length > 0 && (
                                    <div style={{ padding: '12px 16px', background: 'var(--color-surface)', borderRadius: 16, border: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 8, boxShadow: 'var(--shadow-md)', maxHeight: 180, overflowY: 'auto' }}>
                                        ...
                                        {uploadingQueue.map(file => (
                                            <div key={file.id} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                                                <div style={{ width: 14, height: 14, borderRadius: '50%', border: '2px solid var(--color-surface-hover)', borderTopColor: file.status === 'failed' ? 'var(--color-danger)' : (file.status === 'completed' ? '#10b981' : 'var(--color-primary)'), animation: file.status === 'uploading' ? 'chatsPageSpinner 1s linear infinite' : 'none', flexShrink: 0 }} />
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2, fontSize: '11px' }}>
                                                        <span style={{ color: 'var(--color-text)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '80%' }}>{file.name}</span>
                                                        <span style={{ fontWeight: 600, color: file.status === 'failed' ? 'var(--color-danger)' : (file.status === 'completed' ? '#10b981' : 'var(--color-primary)') }}>
                                                    ...
                                                    <div style={{ height: 4, borderRadius: 2, background: 'var(--color-surface-hover)', overflow: 'hidden' }}>
                                                        <div style={{ height: '100%', width: `${file.progress}%`, background: file.status === 'failed' ? 'var(--color-danger)' : (file.status === 'completed' ? '#10b981' : 'var(--color-primary)'), transition: 'width 0.1s ease' }} />
                                                    </div>
                                                </div>
                                            </div>
                                        ))}
                                ...
                                {/* Reply preview */}
                                {replyingTo && (
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 12px', borderRadius: 12, background: 'var(--color-surface-hover)', borderLeft: '4px solid var(--color-primary)', marginBottom: 4 }}>
                                        <div style={{ fontSize: '12px' }}>
                                            <span style={{ fontWeight: 700, display: 'block', color: 'var(--color-text)' }}>Replying to {replyingTo.sender.name}</span>
                                            <span style={{ display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: 'var(--color-text-secondary)', fontSize: '11px' }}>{replyingTo.content || 'Attachment asset'}</span>
                                        </div>
                                        <button onClick={() => setReplyingTo(null)} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 4, color: 'var(--color-text-secondary)' }}>
                                            <X size={14} />
                                        </button>
                                    </div>
                                )}
                                ...
                                <form onSubmit={handleSendMessage} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    ...
                                    <button type="button" onClick={() => fileInputRef.current?.click()} className="btn btn-secondary" style={{ width: 40, height: 40, borderRadius: 10, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }} title="Add attachment">
                                        <Paperclip size={18} />
                                    </button>
                                    ...
                                    <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
                                        <textarea
                                            ...
                                            className="input"
                                            style={{ minHeight: 40, maxHeight: 120, paddingRight: 40, resize: 'none', paddingTop: 10 }}
                                        />
                                        <button type="button" onClick={() => setShowComposerEmoji(!showComposerEmoji)} style={{ position: 'absolute', right: 12, border: 'none', background: 'none', cursor: 'pointer', color: 'var(--color-text-tertiary)' }}>
                                            {showComposerEmoji ? <X size={18} /> : <Smile size={18} />}
                                        </button>

                                        {/* Emoji picker — higher zIndex than reaction picker */}
                                        {showComposerEmoji && (
                                            <div style={{ position: 'absolute', right: 0, bottom: '100%', marginBottom: 12, width: 250, background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 16, boxShadow: 'var(--shadow-xl)', padding: 8, display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 4, zIndex: 70 }}>
                                                {['😀', '😃', '😄', '😁', '😆', '😅', '😂', '🤣', '😊', '😇', '🙂', '🙃', '😉', '😌', '😍', '🥰', '😘', '😗', '😙', '😚', '😋', '😛', '😝', '😜', '🤪', '🤨', '🧐', '🤓', '😎', '🥸', '🥳', '😏', '😒', '😞', '😔', '😟'].map(e => (
                                                    <button key={e} type="button" onClick={() => { setMessageInput(prev => prev + e); setShowComposerEmoji(false); }} style={{ border: 'none', background: 'none', cursor: 'pointer', fontSize: '1.25rem', padding: 4 }}>{e}</button>
                                                ))}
                                            </div>
                                        )}
                                    </div>

                                    <button type="submit" className="btn btn-primary" style={{ width: 40, height: 40, borderRadius: 10, padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                        <Send size={18} />
                                    </button>
                                </form>
                            </div>
                        ...
                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, textAlign: 'center' }}>
                            {sidebarCollapsed && (
                                <button
                                    ...
                                    style={{ marginBottom: 24, gap: 8, display: 'flex', alignItems: 'center' }}
                                >
                                    <MessageSquare size={16} /> Show Chats
                                </button>
                            )}
                            <div style={{ width: 80, height: 80, borderRadius: 24, background: 'var(--color-primary-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-primary)', marginBottom: 24, boxShadow: 'var(--shadow-md)' }}>
                                <MessageSquare size={38} />
                            </div>
                            <h3 style={{ fontSize: '1.25rem', fontWeight: 700, margin: '0 0 8px 0', color: 'var(--color-text)' }}>FlowDesk Chat</h3>
                            <p style={{ fontSize: '0.875rem', color: 'var(--color-text-secondary)', maxWidth: 320, lineHeight: 1.6, margin: 0 }}>
                ...
                {forwardingMessage && (
                    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(4px)', padding: 16 }}>
                        <div className="card" style={{ width: '100%', maxWidth: 440, padding: 24, background: 'var(--color-surface)', borderRadius: 16, border: '1px solid var(--color-border)', boxShadow: 'var(--shadow-xl)', display: 'flex', flexDirection: 'column', maxHeight: '80vh' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                                ...
                                <button onClick={() => { setForwardingMessage(null); setForwardSuccessConvIds([]); }} style={{ border: 'none', background: 'none', cursor: 'pointer', color: 'var(--color-text-secondary)', display: 'flex' }}>
                                    <X size={20} />
                                </button>
                            </div>

                            <div style={{ padding: '10px 12px', background: 'var(--color-bg)', borderLeft: '4px solid var(--color-primary)', borderRadius: 8, fontSize: '11px', color: 'var(--color-text-secondary)', marginBottom: 16, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            ...
                            <div style={{ position: 'relative', marginBottom: 16 }}>
                                ...
                                <input type="text" className="input" placeholder="Search chats..." style={{ fontSize: '0.75rem', paddingLeft: 38, width: '100%', height: 40 }} value={forwardSearchQuery} onChange={(e) => setForwardSearchQuery(e.target.value)} />
                            </div>

                            <div style={{ flex: 1, maxHeight: 250, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6, border: '1px solid var(--color-border)', borderRadius: 12, padding: 8, background: 'var(--color-bg)' }}>
                                {conversations.filter(c => !forwardSearchQuery.trim() || c.name.toLowerCase().includes(forwardSearchQuery.toLowerCase())).map(c => {
                                    const isSent = forwardSuccessConvIds.includes(c._id);
                                    return (
                                        <div key={c._id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 8, borderRadius: 8 }}
                                        ...
                                        >
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                ...
                                                <div>
                                                    ...
                                                    <p style={{ fontSize: '9px', color: 'var(--color-text-tertiary)', margin: 0, textTransform: 'capitalize' }}>{c.type === 'group' ? 'Group Chat' : 'Direct Chat'}</p>
                                                </div>
                                            </div>
                                            {isSent ? (
                                                <span style={{ fontSize: '11px', fontWeight: 700, color: 'var(--color-success)' }}>Sent ✅</span>
                                            ) : (
                                                <button onClick={() => handleForwardMessage(c._id)} className="btn btn-primary" style={{ height: 26, padding: '0 12px', fontSize: '9px', borderRadius: 8, fontWeight: 700 }}>Forward</button>
                                            )}
                                        </div>
                                    );
                                })}
                                {conversations.filter(c => !forwardSearchQuery.trim() || c.name.toLowerCase().includes(forwardSearchQuery.toLowerCase())).length === 0 && (
                                    <div style={{ textAlign: 'center', padding: '24px 0', color: 'var(--color-text-tertiary)', fontSize: '11px' }}>No matching chats found</div>
                                )}
                            </div>

                            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
                                <button onClick={() => { setForwardingMessage(null); setForwardSuccessConvIds([]); }} className="btn btn-secondary" style={{ flex: 1, fontSize: '0.75rem', fontWeight: 700 }}>Done</button>
                            </div>
                        </div>
                    </div>
                )}
