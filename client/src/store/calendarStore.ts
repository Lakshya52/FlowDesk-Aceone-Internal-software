...
export const useCalendarStore = create<CalendarState>((set, get) => ({
  ...
  navigateNext: () => {
    ...
    if (currentView === 'year') set({ currentDate: addMonths(currentDate, 12) });
    else if (currentView === 'month') set({ currentDate: addMonths(currentDate, 1) });
    else if (currentView === 'week') set({ currentDate: addWeeks(currentDate, 1) });
    else if (currentView === 'day' || currentView === 'agenda') set({ currentDate: addDays(currentDate, 1) });
  },
  
  navigatePrev: () => {
    const { currentDate, currentView } = get();
    if (currentView === 'year') set({ currentDate: subMonths(currentDate, 12) });
    else if (currentView === 'month') set({ currentDate: subMonths(currentDate, 1) });
    else if (currentView === 'week') set({ currentDate: subWeeks(currentDate, 1) });
    else if (currentView === 'day' || currentView === 'agenda') set({ currentDate: subDays(currentDate, 1) });
  },
  ...
  openEventModal: (eventId, date) => set({ 
  ...
  openCalendarModal: (calendarId) => set({ 
    isCalendarModalOpen: true, 
    selectedCalendarId: calendarId || null 
  }),
  
  closeCalendarModal: () => set({ 
    isCalendarModalOpen: false, 
    selectedCalendarId: null 
  }),

  isShareModalOpen: false,
  openShareModal: (calendarId) => set({
  ...
  toggleCalendarSidebar: () => set((state) => ({ isCalendarSidebarOpen: !state.isCalendarSidebarOpen })),
  setCalendarSidebarOpen: (isOpen) => set({ isCalendarSidebarOpen: isOpen }),

  isEventDrawerOpen: false,
  openEventDrawer: (eventId) => set({ 
    isEventDrawerOpen: true, 
    selectedEventId: eventId 
  }),
  closeEventDrawer: () => set({ 
    isEventDrawerOpen: false 
